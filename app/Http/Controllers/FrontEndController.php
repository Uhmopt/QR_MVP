...
class FrontEndController extends Controller
{
    ...
    public function qrsaasMode()
    {
        ...
        if (env('DISABLE_LANDING', false)) {
            ...
            $colCounter = [4, 12, 6, 4, 3, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2];
    ...
    public function restorant($alias_restorant, $alias_branch = "")
    {
        $subDomain = $this->getSubDomain();
        if ($subDomain && $alias_restorant !== $subDomain) {
            return redirect()->route('restorant', $subDomain);
        }

        $restorant = Restorant::where('subdomain', $alias_restorant)->first();
        if (!$restorant) {
            abort(404);
            exit;
        }
